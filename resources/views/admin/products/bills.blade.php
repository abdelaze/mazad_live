                ...
                <div class="card-body">
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <h5 class="mb-0">{{  \App\Models\ProductSelectedUser::where('payment_status' , 'paid')->sum('amount')   }} </h5>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <h5 class="mb-0"> {{  \App\Models\ProductSelectedUser::where('payment_status' , 'paid')->sum('paid_amount')   }} </h5>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <h5 class="mb-0"> {{  \App\Models\ProductSelectedUser::where('payment_status' , 'paid')->sum('amount')  -  \App\Models\ProductSelectedUser::where('payment_status' , 'paid')->sum('paid_amount')   }}  </h5>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table id="datatable" class="table  table-bordered table-responsive table-striped   table-hover" data-orderable="0" data-orderable_type="desc">
                            ...
                            <tbody>
                                ...
                                @foreach ($data as $key=>$user)
                                    ...
                                    // dd($user);
                                     ?>
                                    @if(!empty($user->selectedProducts[0]['product'])) 
                                    <tr>
                                        <td>{{ $key+1}}</td>
                                        <td>{{ $user->full_name}} </td>
                                        <td>{{ $user->phone_number}} </td>
                                        <td onclick="window.open('{{ route('products.show', $user->selectedProducts[0]['product']->id) }}','_blank')"
                                            style=" cursor:pointer;">{{ $user->selectedProducts[0]['product']->product_name }}</td>
                                        <td>{{ $user->selectedProducts[0]['amount'] }}</td>
                                        <td>{{ $user->selectedProducts[0]['paid_amount'] }}</td>
                                        <td>{{ $user->selectedProducts[0]['amount']  -  $user->selectedProducts[0]['paid_amount']  }}</td>
                                        <td>

                                            <button class="btn btn-sm btn-danger pay"    data-bs-toggle="modal" data-bs-target="#paid"   data-user_id="{{$user->id}}"  data-id="{{$user->selectedProducts[0]['product']->id}}" > {{   __('translation.Pay') }} </button>
...
<div class="modal fade" id="paid" tabindex="-1" aria-labelledby="paidLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered ">
        <div class="modal-content">
            ...
            <form method="post" action="{{route('product_user.pay.bill')}}" >
                @csrf
