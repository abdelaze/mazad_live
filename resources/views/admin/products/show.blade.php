       ...
       <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">{{ trans('translation.product_details') }}</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-6">
                            <div class="mt-4 mt-xl-3">
                                <a href="javascript: void(0);" class="text-primary">{{ $product->product_name }}</a>
                                <h4 class="mt-1 mb-3">{{ $product->product_desc }}</h4>
                                ...
                                <p class="text-muted mb-4"> {{ trans('translation.views') }}      :  {{ $product->views }} </p>

                                <h5 class="mb-4">{{ trans('translation.price') }}  : {{ $product->price }} {{ $product->currency }} </h5>
                                <h5 class="mb-4">{{ trans('translation.is_sold') }}      : 
                                    
                                    @if($product->is_sold == 1) 
                                          <span class="badge badge-pill badge-soft-success font-size-12">{{  trans('translation.yes') }} </span>
                                     @else 
                                          <span class="badge badge-pill badge-soft-danger font-size-12">{{   trans('translation.not') }} </span>
                                     @endif
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0 table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        <th scope="row" style="width: 400px;"> {{ trans('translation.country') }}</th>
                                        <td>{{ $product->country->name}}</td>
                                    </tr>

                                    <tr>
                                        <th scope="row" style="width: 400px;"> {{ trans('translation.state') }}</th>
                                        <td>{{ $product->state->name}}</td>
                                    </tr>

                                    <tr>
                                        <th scope="row" style="width: 400px;"> {{ trans('translation.city') }}</th>
