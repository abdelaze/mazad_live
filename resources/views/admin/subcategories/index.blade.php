...
@section('title') @lang('translation.subcategories') @endsection
            ...
            <div class="card-body">
                ...
                <div class="row mb-2">
                    <div class="col-sm-4">
                        <div class="search-box me-2 mb-2 d-inline-block">
                            <div class="position-relative">
                                <h4 class="card-title">{{ trans('translation.all_subcategories') }}</h4>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-8">
                        <div class="text-sm-end">
                            <a href="{{ route('subcat-import-view') }}" class="btn btn-info btn-rounded waves-effect waves-light mb-2 me-2"><i class="mdi mdi-plus me-1"></i>   {{ trans('translation.Import_Excel_Sheet') }} </a>
                        </div>
                    </div><!-- end col-->
                </div>
                    ...
                    <div class="table-responsive">
                        <table id="subcat_datatable"  class="table align-middle table-nowrap table-check">
                            ...
                            <tbody id="bodycontent"style="cursor:pointer">
                                ...
                                @foreach ($subcats  as $key=>$subcat)
                                    ...
                                    <tr class="row1" data-id="{{ $subcat->id }}">
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $subcat->name }}</td>
                                        <td>{{ !empty($subcat->category->name)  ? $subcat->category->name  : '' }}</td>
                                        <td style="display:none;">{{ $subcat->position }}</td>
                                        <td> @if($subcat->status == 1)   <button type="button" class="btn btn-outline-success waves-effect waves-light">{{trans('translation.active')}}</button> @else  <button type="button" class="btn btn-outline-danger waves-effect waves-light">{{trans('translation.not_active')}}</button> @endif</td>
                                        <td>
                                            <a class="btn btn-secondary btn-sm edit" href="{{ route('subcategories.edit',$subcat->id) }}"  title="Edit">
                                                <i class="fas fa-pencil-alt"></i>
                                            </a>
                
                                            {!! Form::open(['method' => 'DELETE','route' => ['subcategories.destroy', $subcat->id],'style'=>'display:inline']) !!}
